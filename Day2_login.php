...
if(isset($_POST['event4'])){
    ...
    if(!$result){
        ...
        $q="SELECT COUNT(event4) FROM signup WHERE event4='Y'";
        $r=mysqli_query($connection,$q);
        $row=mysqli_fetch_array($r);
        $total=$row[0];
        if($total>=10){
            $update="UPDATE signup SET event4='' WHERE s_roll={$_SESSION['roll']} ";
            $res_up=mysqli_query($connection,$update);
            echo "<script>alert('ALL SEATS ARE BOOKED,TRY ANOTHER EVENT!!!')</script>";
        }
        else{
...
if(isset($_POST['event5'])){
    ...
    if(!$result){
        ...
        $q="SELECT COUNT(event5) FROM signup WHERE event5='Y'";
        $r=mysqli_query($connection,$q);
        $row=mysqli_fetch_array($r);
        $total=$row[0];
        if($total>=10){
            $update="UPDATE signup SET event5='' WHERE s_roll={$_SESSION['roll']} ";
            $res_up=mysqli_query($connection,$update);
            echo "<script>alert('ALL SEATS ARE BOOKED,TRY ANOTHER EVENT!!!')</script>";
        }
        else{
...
if(isset($_POST['event6'])){
    ...
    if(!$result){
        ...
        $q="SELECT COUNT(event6) FROM signup WHERE event6='Y'";
        $r=mysqli_query($connection,$q);
        $row=mysqli_fetch_array($r);
        $total=$row[0];
        if($total>=10){
            $update="UPDATE signup SET event6='' WHERE s_roll={$_SESSION['roll']} ";
            $res_up=mysqli_query($connection,$update);
            echo "<script>alert('ALL SEATS ARE BOOKED,TRY ANOTHER EVENT!!!')</script>";
        }
        else{
...
$q1="SELECT COUNT(event1) FROM signup WHERE event4='Y'";
$r=mysqli_query($connection,$q1);
$row=mysqli_fetch_array($r);
$total1=$row[0];

$q2="SELECT COUNT(event2) FROM signup WHERE event5='Y'";
$r=mysqli_query($connection,$q2);
$row=mysqli_fetch_array($r);
$total2=$row[0];

 $q3="SELECT COUNT(event3) FROM signup WHERE event6='Y'";
...
<div class="row">
   <div class="column" >
    ...
    <p><?php echo "Filled: ".$total1 ;
        echo "<br>";
        $rem1=10-$total1;
        echo "Remaining: ".$rem1 ;?></div></div>
    </div>
  ...
  <div class="column">
   <div class="container">
    ...
    <p>Venue: SEMINAR HALL</p>
     <p>Time: 06:00-08:00</p>
     <p><?php echo "Filled: ".$total2 ;
        echo "<br>";
        $rem2=10-$total2;
        echo "Remaining: ".$rem2;?></div></div>
      </div>
  ...
  <div class="column">
   <div class="container">
    ...
    <p>Participation Fee: Rs.1000(2-5)</p>
        ...
        <p><?php echo "Filled: ".$total3 ;
        echo "<br>";
        $rem3=10-$total3;
        echo "Remaining: ".$rem3 ;?></div></div>
      </div>
